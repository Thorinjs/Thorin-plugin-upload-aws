  ...
  class AwsStorage extends IStorage {
    ...
    save(fileObj) {
      return new Promise((resolve, reject) => {
        ...
        if (!streamObj) {
          return reject(thorin.error('UPLOAD.STORE_FAILED', 'Could not complete the file upload.'));
        }
      ...
      return new Promise((resolve, reject) => {
        ...
        if (typeof opt === 'string' && opt) {
          try {
            ...
            return reject(thorin.error('DATA.INVALID', 'A valid URL is required'));
          }

        } else if (!opt) {
          opt = {};
        }
        if (!opt.bucket) return reject(thorin.error('DATA.INVALID', 'Please specify the bucket name'));
        if (!opt.key) return reject(thorin.error('DATA.INVALID', 'Please specify the key name'));
